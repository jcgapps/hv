...
def normalize_single_token(tok: str) -> str | None:
    """Convierte 'B-7', 'i22', ' 14 ' → '14' (rango 1..75)."""
    if not tok:
        return None
    s = tok.upper().strip()
...
# Inicializa valores por defecto antes de crear los widgets
for c in cartones:
...
with st.sidebar.expander("Cartones activos", expanded=True):
    for c in cartones:
        key = f"card_active::{c['name']}"
        # NO reasignes a session_state; el widget lo hace
